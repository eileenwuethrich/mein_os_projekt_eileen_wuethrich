...
# gleiche Länge sicherstellen, falls nötig dark_triad zufällig samplen
...
# Die Daten zeigen eine realistische Verteilung psychologischer Merkmale — also das, was man in echten Persönlichkeitserhebungen typischerweise erwartet.
# Diese gleichmässige Streuung ist eine gute Basis für deine hypothetische Analyse, denn sie zeigt, dass die Plausibilität der Daten geprüft wurden, bevor das Modelle berechnet wurde.
# Sie sagt noch nichts über Rückfälligkeit aus, sondern nur über die Verteilung des Persönlichkeitsmerkmals Psychopathie.
...
# VISUALISIERUNG
# Psychopathie in Gruppen einteilen
data$psychopathy_group <- cut(data$psychopathy, 
                              breaks = c(1, 2, 3, 4, 5),
                              labels = c("Sehr niedrig", "Niedrig", "Mittel", "Hoch"),
                              include.lowest = TRUE)

# Rückfallrate pro Gruppe berechnen
library(dplyr)
recid_rates <- data %>%
  group_by(psychopathy_group) %>%
  summarise(Rueckfallrate = mean(as.numeric(is_recid), na.rm = TRUE) * 100)

print(recid_rates)

# Balkendiagramm erstellen
barplot(recid_rates$Rueckfallrate,
        names.arg = recid_rates$psychopathy_group,
        col = "gray",
        ylab = "Rückfallrate (%)",
        xlab = "Psychopathie-Gruppe",
        main = "Rückfallrate nach Psychopathie-Niveau")

# Steigen die Balken nach rechts an, bedeutet das:
# -> mit zunehmender Psychopathei steigt das Risiko, rückfällig zu werden.
# Sind die Balken etwa gleich hoch, deutet dies auf keinen klaren Zusammenhang hin.
...
# -> Das heisst, statistisch gesehen kannst man nicht sicher sagen, dass Psychopathie mit Rückfälligkeit zusammenhängt.
